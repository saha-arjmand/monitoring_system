...
class MERGE_FEEDS_HAVE_ONE_EDW:
    ...
    def db_source_to_df(self):
        ...
        for anyItem in df_np:
            list.append([anyItem[1], anyItem[3], anyItem[4]])


        list_np = np.array(list)

        df = pd.DataFrame(list_np, columns=['EDL_FEED_NAME', 'edl_record_count', 'bib_record_count'])
    ...
    def executive(self):
        ...
        # edw_count_df
        edw_count_df = self.db_source_to_df()
        ...
        db_source = self.convert_to_pd_and_update_header(df3)

...
class RECHECK_EDL:
    ...
    def complete_db_edl_date_format_hint(self, feeds_not_in_date_format_db_table):
        if len(feeds_not_in_date_format_db_table) > 0 :
            ...
            print(f'  INSERT INTO EDL_PROBLEM_FEEDS (edl_feed_name, first_date, last_date, details, status, last_follow_up, action_on_report, type) VALUES ("{anyItem}", "20220101", "{self.dateObj.year}{self.dateObj.month}{self.dateObj.day}", "details", "inactive", "follow up from x", "0", "db");')
    ...
    def edl_new_feed_query_information(self):
        ...
        print(f"  Place (recheck_edl_db_feeds_{self.dateObj.year}{self.dateObj.month}{self.dateObj.day}.csv) file in the inpout folder\n") 


    def edl_file_count_query(self, edl_recheck_feeds):
        line.title_between_line("EDL RECHECK DB COUNT QOUERY")
        
        fileEdlQueryObj = EDL_QueryMaker(self.dateObj, edl_recheck_feeds)
        fileEdlQueryObj.db_edl_null_query()

...
class MERGE_NEW_FEEDS:
    ...
    def newEDL_isexist(self):
        ...
        path = f".\input\\new_edl_db_feeds_{self.dateObj.year}{self.dateObj.month}{self.dateObj.day}.csv"
        newFileExistObj.is_file_exist(path)
        return newFileExistObj.path

    def newEDW_isexist(self):
        newFileExistObj = CheckFile(self.dateObj)
        path = f".\input\\new_edw_db_feeds_{self.dateObj.year}{self.dateObj.month}{self.dateObj.day}.xlsx"
        newFileExistObj.is_file_exist(path)
        return newFileExistObj.path
    ...
    def edl_null_feeds_df(self):
        edlNullObj = NullCheck(self.db_source, 'db')
    ...
    def executive(self):
            line.title_between_line("MERGE new EDL & EDW DB Feeds")

            info = "  File Names : "
            info += "\n"
            info += f"  new_edl_db_feeds_{self.dateObj.year}{self.dateObj.month}{self.dateObj.day}.csv"
            info += "\n"
            info += f"  new_edw_db_feeds_{self.dateObj.year}{self.dateObj.month}{self.dateObj.day}.xlsx"
            info += "\n"
            print(info)
            ...
            db_source = self.merge_vertical_with_template(newEdl_v, newEdw_v)

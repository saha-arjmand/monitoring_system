...
class EDW_NULL:
    ...
    def edw_null_feeds_df(self):
        ...
        edwNullObj = NullCheck(self.df, 'db')
    ...
    def complete_db_edw_date_format_hint(self, feeds_not_in_date_format_db_table):
        if len(feeds_not_in_date_format_db_table) > 0 :
            ...
            for anyItem in feeds_not_in_date_format_db_table:
                ...
                print(f'  INSERT INTO EDW_DATE_FORMAT (edl_feed_name, edw_table_name, partition, partition_format, type) VALUES ("{anyItem}", "STG_GEN.TABLENAME", "YES", "YYYYMMDD", "db");')
            print(f"\n  If the table has a problem, query is like this : ")
            print(f'  INSERT INTO EDW_PROBLEM_FEEDS (edl_feed_name, edw_table_name ,first_date, last_date, details, status, last_follow_up, action_on_report, type) VALUES ("{anyItem}" ,"STG_GEN.TABLENAME" ,"20220101", "{self.dateObj.year}{self.dateObj.month}{self.dateObj.day}", "details", "inactive", "follow up from x", "0", "db");')
    ...
    def status_null_feeds(self, null_feeds_list):
        ...
        nullStatusObj.null_status(null_feeds_list, "edw", 'db')

    def remove_problem_feeds_list(self, null_feeds_list):
        list = problemlist.feeds_list_minus_problem_feeds(null_feeds_list, "edw", "db")
        return list
    ...
    def replacing_problematic_values(self):
        replace_df = problemlist.replacing_problematic_values(self.df, self.dateObj, "edw", "0", "db")
        return replace_df
    ...
    def edw_new_feed_query_information(self):
        ...
        print(f"  Place (new_edw_db_feeds_{self.dateObj.year}{self.dateObj.month}{self.dateObj.day}.csv) file in the inpout folder\n") 

    def edw_db_count_query(self, null_list):
        line.title_between_line("EDW DB COUNT QOUERY")

        dbEdlQueryObj = EDW_QueryMaker(self.dateObj, null_list)
        dbEdlQueryObj.db_edw_null_query()
    ...
    def executive(self):
        ...
        self.edw_db_count_query(edw_null_feeds)

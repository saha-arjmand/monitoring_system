...
class MERGE_RECHECK_EDL_FEEDS:
    ...
    def recheckEDL_isexist(self):
        ...
        path = f".\input\\recheck_edl_file_feeds_{self.dateObj.year_file}{self.dateObj.month_file}{self.dateObj.day_file}.csv"
    ...
    def executive(self):
            ...
            info = "  File Name : "
            info += "\n"
            info += f"  recheck_edl_file_feeds_{self.dateObj.year_file}{self.dateObj.month_file}{self.dateObj.day_file}.csv"
            info += "\n"
            print(info)
            ...
            file_source = self.merge_vertical_with_template(readNewFile_to_vertical )
...
class MERGE_RECHECK_EDW_FEEDS:
    ...
    def recheckEDW_isexist(self):
        recheckFileExistObj = CheckFile(self.dateObj)
        path = f".\input\\recheck_edw_file_feeds_{self.dateObj.year_file}{self.dateObj.month_file}{self.dateObj.day_file}.xlsx"
        recheckFileExistObj.is_file_exist(path)
        return recheckFileExistObj.path
    ...
    def feed_list(self):
        ...
        edl_feed_list = self.file_source.to_numpy()
        feedList = []
    ...
    def merge_vertical_with_template(self ,newEdw_v):
        ...
        # EDW
        mergeEDW = MergeDataFrame(self.file_source, newEdw_v, self.dateObj)
    ...
    def executive(self):
            line.title_between_line("MERGE recheck EDW Files Feeds")

            info = "  File Name : "
            info += "\n"
            info += f"  recheck_edw_file_feeds_{self.dateObj.year_file}{self.dateObj.month_file}{self.dateObj.day_file}.csv"
            info += "\n"
            print(info)
